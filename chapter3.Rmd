...
As expected, most of the students live in urban areas, and so more people have high use of alcohol levels in urban areas, but relatively more people have high use of alcohol in rural areas.
...
As expected, less studytime has higher proportion of students who have high use of alcohol.


Next the going out variable:
...
Here the high use of alcohol increases with the amount of going out with friends.


And lastly the adbsences variable:
...
Here it is clear that increase in absences is linked to an increase proportionally high use of alcohol as well.
...
The median of absences is the same whether student is form rural or urban area when there is no high use of alcohol, but the median for absences is higher for students from rural areas if they belong to the high use of alcohol group.


Next few cross-tabulations. First I wanted to include a table that includes address, the mean of going out and of course high use of alcohol.
...
The mean of going out increases quite a lot when the alcohol use is high for both rural and urban area students. The mean of going out in both cases of high use of alcohol (true or false) is higher for students who live in urban areas compared to rural area students, as would be expected.
...
# model with all the variables: absences, studytime, goout and address
...
When first looking at the summary of the fitted model, all the coefficients are statistically significant at the 5 % level. As the exploration of the variables earlier imlplied, the relationship is positive with absences and goout, so increase in these variables will increase the odds of the student having high use of alcohol. Studytime and address on the other hand decrease the odds of having high use of alcohol.


I looked at first a simple model, where absences was the only explanatory variable, and the AIC was 438.52. After adding the three other variables, the AIC had decreased to 381.44, so adding the other variables has improved the variable.


Looking at the coefficients more closer, absences has an odds ratio of 1.069, meaning that a unit increase in absences increase the odds of the student having high use of alcohol by 6.9% keeping other variables constant, and the confidence interval is between 2.5% and 11.97%. This isn't so large effect, as could be expected based on the graphs in the start.
...
The training error is 0.2297, so on average 23% of the predictions are wrong.
...
The 10-fold cross-validation gives the prediction error of 0.2486, so 24.86% of the predictions are wrong. This is slightly smaller than the model in the exercise set had, 0.26, so the model that I have used has a bit better test set performance.
