...
Surf variable is also significantly correlating with attitude and deep, but with more significance with men.
...
In this model the dependent variable is exam points as instructed. The three explanatory variables that I chose are attitude, gender and stra. In the summary of the model it can be seen that only the intercept and the variable attitude are statistically significant. The coefficient on attitude, 3.5, tells that one unit change in attitude is related to a 3.5 point increase in exam points. The gender variable is a dummy variable that has the value 1 when the person's gender is male, so the coefficient on the variable gender describes the difference between the two genders, and when the person is male, it lowers the exam points by -0.22. The stra variable that is computed from strategic questions has a coefficient of 0.89, meaning that better strategic learning will increase the exam points.The F-test that all three coefficients would be zero has a very small p-value, so it can be rejected.

As the variables gender and stra were not statistically significant, I tried first to drop the gender variable.
...
The attitude variable is statistically highly significant. The attitude variable is positively related to the exam points, with 3.5 increase with one unit change in attitude for the better. The intercept of this model means that if attitude would be 0, exam points would be 11.6 according to this model. What is interesting, is that when gender variable is removed from the model, the stra variable becomes significant at 10 percent level. It has a coefficient 0.9, meaning that one unit increase in strategic learning is related to a 0.9 increase in exam points.


The multiple R-squared is 0.20, which means that this model explains 20 percent of the variation in exam points.

Compared to the last mode, the F-statistic has risen as well, and the p-value is smaller.
...
The first plot that compares the residuals with the fitted values shows that the points are spread around quite randomly, except for a few outliers, so the model is quite appropriate. The plot does not indicate non-linear trends or non-constant variance.
...
The residuals vs leverage plot shows if there are any outliers that would be significant for the model. The Cook's distance curved lines don't show in this plot, so the outliers aren't too disturbing and there aren't points that would have high residuals and too much leverage at the same time.
